#ifndef _INIFILE_H
#define _INIFILE_H

#define INIFILE_VERSION "1.0.0"

// Maximum length for filename, excluding NULL char 26 chars allows an
// 8.3 filename instead and 8.3 directory with a leading slash
#define INI_FILE_MAX_FILENAME_LEN 26

#include <Arduino.h>
#include "WaveHC.h"

class IniFileState;

class WaveShieldIniFile {
public:
  enum error_t {
    errorNoError = 0,
    errorFileNotFound,
    errorFileNotOpen,
    errorBufferTooSmall,
    errorSeekError,
    errorSectionNotFound,
    errorKeyNotFound,
    errorEndOfFile,
    errorUnknownError,
  };

  static const uint8_t maxFilenameLen;

  // Create an IniFile object. It isn't opened until open() is called on it.
  WaveShieldIniFile(FatReader& file, 
	  bool caseSensitive = false);
  ~WaveShieldIniFile();

  inline bool open(FatReader& , const char* ); // Returns true if open succeeded
  inline void close(void);

  inline bool isOpen(void) ;

  inline error_t getError(void) ;
  inline void clearError(void) ;

  // Get the filename asscoiated with the ini file object
  inline const char* getFilename(void) ;

  bool validate(char* buffer, size_t len) ;
  
  // Get value from the file, but split into many short tasks. Return
  // value: false means continue, true means stop. Call getError() to
  // find out if any error
  bool getValue(const char* section, const char* key,
		   char* buffer, size_t len, IniFileState &state) ;

  // Get value, as one big task. Return = true means value is present
  // in buffer
  bool getValue(const char* section, const char* key,
		  char* buffer, size_t len) ; 

  // Get the value as a string, storing the result in a new buffer
  // (not the working buffer)
  bool getValue(const char* section, const char* key,
		   char* buffer, size_t len, char *value, size_t vlen) ;

  // Get a boolean value
  bool getValue(const char* section, const char* key,
		   char* buffer, size_t len, bool& b) ;

  // Get an integer value
  bool getValue(const char* section, const char* key,
		   char* buffer, size_t len, int& val) ;

  // Get a uint16_t value
  bool getValue(const char* section, const char* key,
		   char* buffer, size_t len, uint16_t& val) ;

  // Get a long value
  bool getValue(const char* section, const char* key,
		   char* buffer, size_t len, long& val) ;

  bool getValue(const char* section, const char* key,
		   char* buffer, size_t len, unsigned long& val) ;

  // Get a float value
  bool getValue(const char* section, const char* key,
		   char* buffer, size_t len, float& val) ;
  
 

  // Utility function to read a line from a file, make available to all
  //static int8_t readLine(File &file, char *buffer, size_t len, uint32_t &pos);
   error_t readLine(char *buffer, size_t len, uint32_t &pos);
   bool isCommentChar(char c);
   char* skipWhiteSpace(char* str);
   void removeTrailingWhiteSpace(char* str);

  bool getCaseSensitive(void) ;
  void setCaseSensitive(bool cs);
  
  protected:
  // True means stop looking, false means not yet found
  bool findSection(const char* section, char* buffer, size_t len,	
		      IniFileState &state) ;
  bool findKey(const char* section, const char* key, char* buffer,
		 size_t len, char** keyptr, IniFileState &state) ;


private:
  mutable error_t _error;
   FatReader& _file;
  bool _caseSensitive;
};

bool WaveShieldIniFile::open(FatReader& root, const char* filename)
{
    
   _file.open(root,(char*)filename);
  if (_file.isOpen()) {
    _error = errorNoError;
    return true;
  }
  else {
    _error = errorFileNotFound;
    return false;
  }
}

void WaveShieldIniFile::close(void)
{
  _file.close();
}

bool WaveShieldIniFile::isOpen(void) 
{
  return (_file.isOpen());
}

WaveShieldIniFile::error_t WaveShieldIniFile::getError(void) 
{
  return _error;
}

void WaveShieldIniFile::clearError(void) 
{
  _error = errorNoError;
}







class IniFileState {
public:
  IniFileState();

private:
  enum {funcUnset = 0,
	funcFindSection,
	funcFindKey,
  };
	
  uint32_t readLinePosition;
  uint8_t getValueState;

  friend class WaveShieldIniFile;
};

  
#endif

